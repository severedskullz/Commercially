...
namespace Vinconomy.GUI
{
    public class GuiVinconShopCatalog : GuiDialogGeneric
    {
        ...
        public GuiVinconShopCatalog(string DialogTitle, ShopCatalog catalog, List<ShopCatalog> shopList, ICoreClientAPI capi): base(DialogTitle, capi)
        {
            ...
            this.DialogTitle = catalog.Ownable.Name;
            this.ShopList = shopList;

            List<ShopProduct> products = catalog.ProductList.Products;
            ...
            {
                ProductInventory[index].Itemstack = VinUtils.DeserializeItem(capi, product.ProductCode, product.TotalStock, product.ProductAttributes);
                CurrencyInventory[index].Itemstack = VinUtils.DeserializeItem(capi, product.CurrencyCode, product.CurrencyQuantity, product.CurrencyAttributes);
                index++;
            }
        ...
        private void Compose()
        {
            ...
            try
            {
                ...
                ElementBounds itemSlotBounds = ElementStdBounds.SlotGrid(EnumDialogArea.None, insetWidth, insetHeight, 15, (int)Math.Ceiling(Catalog.ProductList.Products.Count / 15.0f)).WithFixedPosition(15,0).WithParent(itemContainerBounds);
